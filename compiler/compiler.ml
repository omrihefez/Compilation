...
let rec gcd a b =
 ...
 module Reader : READER = struct
   ...
   and nt_paired_comment str = 
     ...
     let nt4 = diff nt_any nt3 in
     let nt4 = disj (unitify nt4) nt2 in
     let nt4 = star nt4 in
     let nt5 = char '}' in
   ...
   and nt_sexpr_comment str = 
     let prefix = word "#;" in
     let nt1 = nt_sexpr in
     let nt2 = nt_sexpr_comment in
     let nt3 = unitify (caten prefix nt1) in
     let nt4 = unitify (disj nt3 (unitify (caten prefix (caten nt2 nt1)))) in
     nt4 str
   and nt_comment str =
     disj_list
       [nt_line_comment;
   ...
   and nt_skip_star str =
     ...
     nt1 str
   and make_skipped_star (nt : 'a parser) = make_make_skipped_star nt_skip_star nt
   and nt_digit str = 
   ...
   and nt_nat str = 
     ...
     let nt1 = pack nt1 (fun digits -> List.fold_left (fun num digit -> 10 * num + digit) 0 digits) in
     nt1 str
   and nt_hex_nat str = 
   ...
   and nt_optional_sign str = 
     ...
     let nt1 = pack (maybe (disj plus_sign minus_sign)) (fun s -> match s with
     | Some('+') -> true
     | Some ('-') -> false
     | _ -> true) in
     nt1 str 
   ...
   and nt_float str = 
     let minus_sign = char '-' in
     let decimal_dot = char '.' in
     let case1 (* -.1e1 *) = pack (caten (caten (caten minus_sign decimal_dot) nt_mantissa) nt_exponent)
                       (fun (((minus, dot), mantissa), exponent) ->
                         let mantissa = -1. *. mantissa in
                         ScmReal(mantissa *. exponent)
 
                       ) in
     let case2 (* .1e1 *) = pack (caten (caten decimal_dot nt_mantissa) nt_exponent)
                       (fun ((dot, mantissa), exponent) ->
                         match mantissa with
                           | 0. -> raise X_no_match
                           | _ -> ScmReal(mantissa *. exponent)
                         ) in
     let case3 (* 1.1e1 *) = pack (caten (caten (caten nt_integer_part decimal_dot) nt_mantissa) nt_exponent)
                       (fun (((int_part, dot), mantissa), exponent) ->
                         let mantissa = if (int_part < 0.) then (-1. *. mantissa) else mantissa in
                         ScmReal((int_part +. mantissa) *. exponent)
                       ) in
     let case4 (* 1e1 *) = pack (caten nt_integer_part nt_exponent)
                       (fun (int_part, exponent) ->
                         match exponent with
                         | 0. -> ScmReal(int_part)
                         | _ -> ScmReal(int_part *. exponent)
                         ) in
     let case5 (* 1.1*) = pack (caten nt_integer_part (caten decimal_dot nt_mantissa))
                         (fun (int_part,(_, mantissa)) -> ScmReal(int_part +. mantissa)) in
     let float_number = disj_list [case1; case2; case3; case4; case5] in
     float_number str    
   and nt_number str =
     let nt1 = nt_float in
     let nt2 = nt_frac in
   ...
   and nt_char_named str =
     let make_named_char char_name c = 
       let nt1 = pack (word_ci char_name) (fun _ -> c) in nt1 
     in
     let nt1 = disj_list [
   ...
   and nt_string_part_dynamic str = 
     ...
     let nt1 = caten nt_tilde (caten nt_left_bracket nt_space) in
     let nt1 = pack nt1 (fun (_, (_, _)) -> "~{") in
     let nt1 = caten nt1 (caten nt_sexpr (caten nt_space nt_right_bracket)) in
     let nt1 = pack nt1 (fun (_, (sexpr, (_, _))) -> 
   ...
   and nt_vector str = 
     let nt_right_bracket = char ')' in
     let nt1 = word "#(" in
     let nt2 = caten nt_skip_star nt_right_bracket in
     let nt2 = pack nt2 (fun _ -> ScmVector []) in
     let nt3 = plus nt_sexpr in
     let nt3 = caten nt3 nt_right_bracket in
   ...
   and nt_list str = 
     ...
     let nt_right_bracket = char ')' in
     let nt_dot = char '.' in
     let nt2 = pack (make_make_skipped_star nt_skip_star nt_right_bracket) (fun _ -> ScmNil) in
     let nt3 = plus nt_sexpr in
     let nt4 = pack nt_right_bracket (fun _ -> ScmNil) in
     let nt5 = pack (caten nt_dot (caten nt_sexpr nt_right_bracket)) (fun (_, (sexpr, _)) -> sexpr) in
     let nt4 = disj nt4 nt5 in
     let nt3 = pack (caten nt3 nt4) (fun (sexprs, sexp) ->
                     List.fold_right
...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec macro_expand_let ribs exprs =
    ...
    | _ ->
      let rec calc_vars = function
        | ScmNil -> ScmNil
        | ScmPair (ScmPair (var, _), rest) -> ScmPair (var, calc_vars rest)
        | _ -> raise (X_syntax "malformed let-rib") in
      let vars = calc_vars ribs in
      let rec calc_vals = function
        | ScmNil -> ScmNil
        | ScmPair (ScmPair (_, ScmPair(v, ScmNil)), rest) -> ScmPair (v, calc_vals rest)
        | _ -> raise (X_syntax "malformed let-rib") in
      let vals = calc_vals ribs in
  ...
  let rec macro_expand_letrec ribs exprs =
    ...
    | _ ->
      let rec calc_vars = function
        | ScmNil -> ScmNil
        | ScmPair (ScmPair (var, ScmPair (value, ScmNil)), ScmNil) -> ScmPair (ScmPair (var, ScmPair (ScmSymbol "'whatever", ScmNil)), ScmNil)
        | ScmPair (ScmPair (var, ScmPair (value, ScmNil)), rest) -> ScmPair (ScmPair (var, ScmPair (ScmSymbol "'whatever", ScmNil)), calc_vars rest)
        | _ -> raise (X_syntax "malformed letrec-rib") in
      let vars = calc_vars ribs in
      let rec calc_vals ribs body = match ribs with
        | ScmNil -> body
        | ScmPair (ScmPair (var, ScmPair (value, ScmNil)), ScmNil) -> ScmPair (ScmPair (ScmSymbol "set!", ScmPair (var, ScmPair (value, ScmNil))), body)
        | ScmPair (ScmPair (var, ScmPair (value, ScmNil)), rest) -> ScmPair (ScmPair (ScmSymbol "set!", ScmPair (var, ScmPair (value, ScmNil))), calc_vals rest body)
        | _ -> raise (X_syntax "malformed letrec-rib") in
      let vals = calc_vals ribs exprs in
      ScmPair (ScmSymbol ("let"), ScmPair (vars, vals));;
    ...
    | ScmPair (proc, args) ->
       let proc =
         (match proc with
          | ScmSymbol var ->
             if (is_reserved_word var)
             then raise (X_syntax (Printf.sprintf "%s is a reserved word in proc position" var))
             else proc
          | proc -> proc) in
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_lexical_address =
    let rec run expr params env =
      ...
      | ScmVarDef(Var v, expr) -> ScmVarDef' (Var' (v, Free),
                                          run expr params env)
  ...
  | [] -> raise (X_this_should_not_happen "var must occur in some rib")
  | rib :: ribs ->
    if (List.mem name rib)
    then rib
    else find_var_rib name ribs;;
  ...
  let should_box_var name expr params = 
    ...
    let rec run = function
      | [] -> false
      | ((Var' (name1, Param _), _), (Var' (name2, Param _), _)) :: rest ->
        run rest
      | ((Var' (name1, Param _), _), (Var' (name2, Bound _), _)) :: rest -> true
      | ((Var' (name1, Bound _), _), (Var' (name2, Param _), _)) :: rest -> true
      | ((Var' (name1, Bound _), env1), (Var' (name2, Bound _), env2)) :: rest ->
...
module Code_Generation : CODE_GENERATION = struct
  ...
  let collect_constants = function
    | [] -> []
    | expr' :: exprs' ->
       let rec run = function
       ...
       and runs exprs' =
          List.fold_left (fun found expr -> found @ (run expr)) [] exprs' in
       run expr' @ (runs exprs');;
  ...
  let search_constant_address = 
    let rec run sexpr = function
      | [] -> raise (X_this_should_not_happen "The constant was not found in the table")
      | (const, loc, _) :: consts ->
         if sexpr = const then loc else (run sexpr consts) in
    run;;
  ...
  let make_lambda_opt_build_list =
    make_make_label ".L_lambda_opt_build_list";;
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmVarSet' (Var' (v, Param minor), expr') ->
        (run params env expr') ^
         "\tmov PARAM(" ^ (string_of_int minor) ^ "), rax\n
          \tmov rax, sob_void\n"
      | ScmVarSet' (Var' (v, Bound (major, minor)), expr') ->
        (run params env expr') ^
          "\tmov r8, ENV\n" ^
          "\tmov r8, qword [r8 + 8 * " ^ (string_of_int major) ^ "]\n" ^
          "\tmov qword [r8 + 8 * " ^ (string_of_int minor) ^ "], rax\n" ^
          "\tmov rax, sob_void\n"
      ...
      | ScmBox' (Var' (v, Param minor)) -> 
          ...
          "\tpop r8\n" ^
          "\tmov qword [r8], rax\n" ^
          "\tmov PARAM(" ^ (string_of_int minor) ^ "), r8\n" ^
          "\tmov rax, r8\n"
      | ScmBox' _ -> raise (X_this_should_not_happen "Should not be boxed!")
      | ScmBoxGet' var' ->
      ...
      | ScmLambda' (params', Opt opt, body) ->
        ...
        and label_build_list = make_lambda_opt_build_list ()
        in
        "\tmov rdi, (1 + 8 + 8)\t; sob closure\n" ^
        "\tcall malloc\n" ^
        "\tpush rax\n" ^
        ...
        "\tmov rsi, rax\n" ^
        "\tpush rax\n" ^
        (Printf.sprintf "\tlea rbx, [r10 - %d]\n" (List.length params')) ^
        (Printf.sprintf "\tlea rcx, [rsp + (1 + 1 + 1 + 1 + %d) * 8]\n" (List.length params')) ^
        (Printf.sprintf "%s:\n" label_build_list) ^
        "\tmov rdi, 1 + 8 + 8\n" ^
        "\tcall malloc\n" ^
        "\tmov [rsi], rax\n" ^
        ...
        (Printf.sprintf "\tje %s\n" label_loop_exit) ^
        (Printf.sprintf "\tmov rcx, 3 + %d\n" (List.length params')) ^
        "\tlea rax, [r9 - 8 * 1]\n" ^
        "\tmov rdx, r10\n" ^
        "\tneg rdx\n" ^
        (Printf.sprintf "\tlea rbx, [rax + 8 * (rdx + %d)]\n" ((List.length params') + 1)) ^
        (Printf.sprintf "%s: \n" label_loop) ^
        "\tmov rdx, qword [rbx]\n" ^
        "\tmov qword [rax], rdx\n" ^
        "\tsub rax, 8\n" ^
      ...
      | ScmApplic' (proc, args, Non_Tail_Call) ->
        ...
        let args_code = List.map (fun arg -> (run params env arg)) args in
        let args_code = List.map (fun code -> code ^ "\tpush rax\n") args_code in
        let args_code = List.rev args_code in (* arguments are pushed in reverse order *)
        let args_code = String.concat "" args_code in
        args_code ^
        "\tpush " ^ n ^ "\t; push number of args\n" ^
        "\t; ~NTC Application - run proc code~\n" ^
        (run params env proc) ^
        "\t; ~end of NTC Application - run proc code~\n" ^
        "\tcmp byte [rax], T_closure\n" ^
        "\tjne L_error_non_closure\n" ^
        "\tpush SOB_CLOSURE_ENV(rax)\t; push env\n" ^
        "\tcall SOB_CLOSURE_CODE(rax)\t; call code\n"
      | ScmApplic' (proc, args, Tail_Call) -> 
        ...
        args_code ^
        "\tpush " ^ n ^ "\t; push number of args\n" ^
        "\t; ~TC Application - run proc code~\n" ^
        (run params env proc) ^
        "\t; ~end of TC Application - run proc code~\n" ^
        "\tcmp byte [rax], T_closure\n" ^
        "\tjne L_error_non_closure\n" ^
        "\tpush SOB_CLOSURE_ENV(rax)\t; push env\n" ^
        ...
        (* fix the stack *)
        Printf.sprintf "\tmov rcx, (%d + 4)\t; rcx = num of args + 4 (env, ret addr, old rbp, num of args)\n" (List.length args) ^
        "\tlea rsi, [rbp - 8 * 1]\t; rsi = top of new frame\n" ^
        "\tmov rdi, COUNT\t; rdi = num of old args\n" ^
        "\tlea rdi, [rbp + 8 * rdi + 8 * 3]\t; rdi = top of the old frame\n" ^
        Printf.sprintf "%s:\t; start recycle frame loop\n" label_recycle_frame_loop ^
        "\tcmp rcx, 0\n" ^
        "\tje " ^ label_recycle_frame_done ^ "\n" ^
        "\tmov r9, qword [rsi]\t; r9 <- arg to copy\n" ^
        "\tmov qword [rdi], r9\t; copy arg\n" ^
        "\tsub rsi, 8 * 1\n" ^
        "\tsub rdi, 8 * 1\n" ^
        "\tsub rcx, 1\n" ^
        "\tjmp " ^ label_recycle_frame_loop ^ "\n" ^
        Printf.sprintf "%s:\t; done recycle frame\n" label_recycle_frame_done ^
        "\tlea rsp, [rdi + 8*1]\t; fix rsp\n" ^
        "\tpop rbp\t; restore old rbp\n" ^
        "\tjmp SOB_CLOSURE_CODE(rax)\t; jump to code\n"
  ...
  let compile_and_run_scheme_string file_out_base user =
    ...
    (string_to_file (Printf.sprintf "%s.asm" file_out_base) asm_code;
      ...
      | n -> (Printf.printf "!!! Compilation failed with code %d\n" n; ()));;
