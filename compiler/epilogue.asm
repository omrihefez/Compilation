...
.L_apply_reverse_list_loop:
        cmp rsi, sob_nil
        je .L_apply_push_reverse_list_loop
        mov rdi , 1+8+8
        call malloc
        mov byte [rax], T_pair
        ...
        inc r13
        jmp .L_apply_reverse_list_loop
        
.L_apply_push_reverse_list_loop:
        cmp r15, sob_nil
        je .L_apply_push_reverse_list_loop_finish
        mov rsi, SOB_PAIR_CAR(r15)
        push rsi
        mov r15, SOB_PAIR_CDR(r15)
        jmp .L_apply_push_reverse_list_loop
        
.L_apply_push_reverse_list_loop_finish:
        dec r8
        mov r10, r8
        
.L_apply_fix_elements_loop:
        cmp r10, 0
        je .L_apply_fix_elements_loop_finish
        push PARAM(r10)
        dec r10
        jmp  .L_apply_fix_elements_loop
        
.L_apply_fix_elements_loop_finish:
        add r13, r8
        push r13
        mov r14, PARAM(0)
        mov rcx, SOB_CLOSURE_ENV(r14)
        push rcx                                
        push RET_ADDR
        add r13, 3
...
.L_apply_fix_frame_loop:
        cmp r9, 0
        jz .L_apply_fix_frame_loop_finish
        dec r8
        mov rcx, rbp
        sub rcx, r10
        mov rcx, qword [rcx]
        mov qword [rbp + r8 * 8], rcx
        add r10, 8
        dec r9
        jmp .L_apply_fix_frame_loop
        
.L_apply_fix_frame_loop_finish:
        lea rsp, [rbp + r8 * 8]
        mov rbp, r12
        ; jump to code pointer
        mov r13, SOB_CLOSURE_CODE(r14)
        jmp r13
